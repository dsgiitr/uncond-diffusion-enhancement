...
def main():
    parser = argparse.ArgumentParser(description="Evaluate RMS Contrast on a folder of images")
    ...
    print(f"Evaluating RMS Contrast on {len(image_paths)} images in '{folder_path}'")
    ...
    for img_path in tqdm(image_paths, desc="Processing images"):
        try:
            ...
            # RMS contrast is standard deviation of pixel intensities
            # Typically normalized to 0-1 range
            img_normalized = img.astype(np.float32) / 255.0
            score = img_normalized.std()
    ...
    if valid_count > 0:
        ...
        print(f"Average RMS Contrast: {avg_score:.4f} (0-1 scale)")
    else:
