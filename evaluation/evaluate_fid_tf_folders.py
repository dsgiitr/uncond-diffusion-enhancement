...
def read_batches_from_dir(dir_path: str, batch_size: int, num_samples: int = None) -> Iterable[np.ndarray]:
    import glob
    from PIL import Image
    import numpy as np
    import random
    ...
    if os.path.exists(os.path.join(dir_path, "dataset_info.json")) or len(glob.glob(os.path.join(dir_path, "*.arrow"))) > 0:
        ...
        batch = []
        for item in split:
            # HuggingFace typically uses 'image' but may use 'img'
            ...
            if len(batch) == batch_size:
                yield np.stack(batch, axis=0) # Shape: (B, H, W, 3)
                batch = []
    ...
    # Fallback: standard image folder (for generated datasets)
...
def main():
    ...
    parser.add_argument("ref_path", help="path to reference images directory OR pre-computed .npz stats file")
    parser.add_argument("sample_dir", help="path to sample images directory", nargs="?")
    parser.add_argument("--save-stats", type=str, help="path to save computed reference stats and activations as .npz")
    ...
    # 1. Provide reference statistics & activations
    if args.ref_path.endswith('.npz'):
        print(f"loading pre-computed reference statistics and activations from {args.ref_path}...")
        obj = np.load(args.ref_path)
        ref_stats = FIDStatistics(obj["mu"], obj["sigma"])
        ref_stats_spatial = FIDStatistics(obj["mu_s"], obj["sigma_s"])
        
        if "pool_acts" in obj:
            ref_pool_acts = obj["pool_acts"]
        else:
            print("Warning: 'pool_acts' not found in .npz. Precision/Recall will be skipped.")
            ref_pool_acts = None
    else:
        print(f"computing reference batch activations from {args.ref_path}...")
        ref_acts = evaluator.read_activations_from_dir(args.ref_path, num_samples=args.num_samples)
        ref_pool_acts = ref_acts[0]
        ...
        if args.save_stats:
            print(f"saving reference stats and activations to {args.save_stats}...")
            np.savez(
                args.save_stats,
                mu=ref_stats.mu,
                ...
                pool_acts=ref_acts[0]  # Required for Precision and Recall
            )
            print("Done saving.")

    # 2. Check if we have sample models to evaluate
    if not args.sample_dir:
        if not args.save_stats:
            print("No sample directory or --save-stats provided. Nothing else to do.")
        return

    # 3. Compute sample statistics & activations
    print(f"computing sample batch activations from {args.sample_dir}...")
    sample_acts = evaluator.read_activations_from_dir(args.sample_dir, num_samples=args.num_samples)
    ...
    if ref_pool_acts is not None:
        prec, recall = evaluator.compute_prec_recall(ref_pool_acts, sample_acts[0])
        print("Precision:", prec)
        print("Recall:", recall)
    else:
        print("Precision: N/A (Reference activations unavailable)")
        print("Recall: N/A (Reference activations unavailable)")
...
class Evaluator:
    ...
    def read_activations_from_dir(self, dir_path: str, num_samples: int = None) -> Tuple[np.ndarray, np.ndarray]:
        return self.compute_activations(read_batches_from_dir(dir_path, self.batch_size, num_samples))
