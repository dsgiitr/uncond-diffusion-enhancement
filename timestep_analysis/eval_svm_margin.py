...
using the direct geometric width of a Linear SVC.
"""
...
def parse_args():
    parser = argparse.ArgumentParser("SVM Margin Evaluator")
...
def main():
    ...
    log_file = out_dir / f"{concept_name}_svm_margin_log.txt"
    ...
    for ts in timesteps:
        ...
        logger.info(f" TS: {ts:04d} | Shape: {tuple(p_data.shape)} -> Flat: {X.shape[1]} | Geometric Margin: {margin:>.5f}")
    ...
    plt.plot(timesteps, scores, marker="o", color="green", linewidth=2)
    plt.xlim(max(timesteps) + 50, min(timesteps) - 50)
    plt.title(f"SVM Geometric Margin Width: {concept_name}", fontsize=14, pad=10)
    plt.xlabel("Diffusion Timestep", fontsize=12)
    plt.ylabel("Margin Width (2 / ||w||)", fontsize=12)
    plt.grid(True, linestyle="--", alpha=0.6)

    out_plot_name = out_dir / f"{concept_name}_svm_margin_plot.png"
